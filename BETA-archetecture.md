...
graph TB
    ...
    subgraph L1 ["LAYER 1 — METADATA SHIELD (Nobody Sees Your IP)"]
        ...
        SDK["NOFACE SDK<br>Generates ZK-Intents<br>Manges secret notes<br>No gas required"]:::meta
    ...
    subgraph L2 ["LAYER 2 — THE UNIVERSAL VAULT"]
        ...
        NofaceVault["NofaceVault.sol<br>Master State Controller<br>Issues private notes<br>Never knows who owns what"]:::vault
    ...
    subgraph L4 ["LAYER 4 — INTENT ENGINE (Gasless Execution)"]
        ...
        subgraph Solvers ["Solver Network"]
            SolverA["Solver A<br>Stakes $NOFACE"]:::intent
    ...
    subgraph L5 ["LAYER 5 — PRIVATE SWAP ENGINE (Uniswap v4)"]
        Hook["NofaceHook.sol<br>Uniswap v4 Hook<br>TSTORE privacy routing<br>Pulls from vault → AMM → vault"]:::hook
    ...
    subgraph L7 ["LAYER 7 — $NOFACE ECONOMIC ENGINE"]
        ...
        subgraph FeeSplit ["Fee Distribution"]
            ...
            BuyBurn["0.1% → Buyback & Burn $NOFACE"]:::token
            Treasury["0.1% → Protocol Treasury"]:::token
        end

        Staking["StakingContract.sol<br>Lock $NOFACE → Earn yield + voting power"]:::economy
        DAO["NOFACE DAO<br>Governs fees, assets, compliance"]:::economy
    end
    ...
    %% Vault
    AssetVaults --> NofaceVault
    SDK --> NofaceVault
    NofaceVault --> Tree & Nullifiers & RootHistory
    ...
    %% Private Economy
    NofaceVault & Hook --> BettingEngine & TournEngine & YieldEngine & CommunityEngine
